...
private:
    ...
    void loadModel(const string& path)
    {
        ...
        const aiScene* scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_TransformUVCoords | aiProcess_FlipUVs);
        // check for errors
        ...
        for(unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {   // if texture hasn't been loaded already, load it
                ...
                if(texPath && texPath[0] == '*')
                {
                    ...
                    if(endptr != (texPath + 1) && scene->mTextures && texIndex >= 0 && texIndex < (int)scene->mNumTextures)
                    {
                        ...
                        if(atex->mHeight == 0)
                        {
                            ...
                            data = stbi_load_from_memory(reinterpret_cast<unsigned char*>(const_cast<void*>(reinterpret_cast<const void*>(atex->pcData))), (int)size, &width, &height, &nrComponents, 0);
                        }
                        else
                        {
