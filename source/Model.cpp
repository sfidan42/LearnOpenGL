...
bool Model::loadGeometry()
{
	...
	constexpr size_t n = sizeof(verts) / (8 * sizeof(float));
	vertices.resize(n);
	for(uint32_t i = 0; i < n; i++)
	{
		const glm::vec3 position(verts[i * 8 + 0], verts[i * 8 + 1], verts[i * 8 + 2]);
		const glm::vec3 normal(verts[i * 8 + 3], verts[i * 8 + 4], verts[i * 8 + 5]);
		const glm::vec2 texCoord(verts[i * 8 + 6], verts[i * 8 + 7]);
...
bool Model::setMaterial(const std::shared_ptr<Material>& mat)
{
	if(!mat)
		return false;
	material = mat;
...
void Model::bind() const
{
	...
	// Setup instance attribute (mat4 model)
...
void Model::send(const Shader& shader) const
{
	if (material)
...
void Model::draw() const
{
	glBindVertexArray(VAO);
	glBindBuffer(GL_ARRAY_BUFFER, instanceVBO);
	glBufferData(GL_ARRAY_BUFFER, static_cast<GLsizeiptr>(instances.size() * sizeof(InstanceData)), instances.data(),
				 GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
